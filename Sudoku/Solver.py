class Solver:
    ...
    def solve(self):
        ...
        # NEW: recursive uniqueness checker
    def count_solutions(self, index=0):
        if index == len(self.vacants):
            self.solutions += 1
            return
        current = self.vacants[index]
        for x in range(1, 10):
            if x in self.board.get_possibles(current):
                current.value = x
                self.count_solutions(index + 1)
                if self.solutions > 1:  # early exit if >1 solution
                    return
        current.value = 0
